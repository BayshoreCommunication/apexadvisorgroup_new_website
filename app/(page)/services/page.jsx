...
const page = () => {
  const ServiceCard = ({ el, isReversed }) => (
    <div
      className={`flex-col ${isReversed ? "lg:flex-row-reverse" : "lg:flex-row"} flex gap-6 pb-10 border-b-1 mb-10`}
    >
      <div className="lg:max-w-[23%] w-full hover:brightness-75 hover:scale-105 duration-700 mb-6 lg:mb-0">
        <Image
          src={`/image/${el.cardImage}`}
          alt={el?.cardTitle || "service image"}
          width={270}
          height={270}
          className="w-full"
        />
      ...
      <div>
        <h1 className="text-[32px] font-bold mb-4">{el?.cardTitle}</h1>
